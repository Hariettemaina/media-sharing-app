...
//         // Convert the image data to a DynamicImage for processing
...
//         // Save the optimized image to the system
//         let filepath = format!("{}/{}", user_uploads_dir, filename);
//         // Detect the original format of the image
//         let original_format = img.format();
//         // Save the image in its original format
//         if let Err(e) = img.save_with_format(&filepath, original_format) {
//             log::error!("Failed to save optimized image: {}", e);
//             return Err(async_graphql::Error::new(format!(
...
//                 images::description.eq(None::<String>),
//                 images::exif_data.eq(None::<String>),
//                 images::format.eq(format!("image/{}", original_format.to_string().to_lowercase())),
//                 images::size.eq(fs::metadata(&filepath)?.len() as i32),
//                 images::width.eq(800), 
//                 images::height.eq(800),
//                 images::created_at.eq(time_now),
//                 images::deleted_at.eq(None::<NaiveDateTime>),
//             ))
//             .execute(&mut conn)
...
// // // Assuming you have the original file extension in a variable `original_extension`
// // let original_format = match original_extension {
// //     "jpg" | "jpeg" => ImageFormat::Jpeg,
// //     "png" => ImageFormat::Png,
// //     "gif" => ImageFormat::Gif,
// //     "bmp" => ImageFormat::Bmp,
// //     _ => ImageFormat::Png, // Default to PNG if the format is not recognized
// // };
...
// // // Save the image in its original format
// // if let Err(e) = img.save_with_format(&filepath, original_format, 80) {
// //     log::error!("Failed to save optimized image: {}", e);
// //     return Err(async_graphql::Error::new(format!(
// //         "Failed to save optimized image: {}",
// //         e
